...
import { Search, Filter, Clock, Calendar, Tag, List, LayoutGrid } from "lucide-react"
...
const mockPosts = [
  {
    ...
    excerpt:
      "Discover how to reclaim your time and attention in an increasingly connected world. Learn practical strategies for digital wellness.",
    publishedAt: "2025-01-15",
    readingTime: "8 min read",
    category: "Digital Wellness",
    tags: ["mindfulness", "productivity", "wellness"],
    image:
      "https://images.pexels.com/photos/267350/pexels-photo-267350.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "large", // Featured post
  },
  {
    id: 2,
    title: "The Psychology Behind Social Media Scrolling",
    excerpt:
      "Understanding the mechanisms that keep us hooked to our devices and how to break free from endless scrolling patterns.",
    publishedAt: "2025-01-12",
    readingTime: "12 min read",
    category: "Psychology",
    tags: ["psychology", "social media", "behavior"],
    image:
      "https://images.pexels.com/photos/607812/pexels-photo-607812.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "medium",
  },
  {
    id: 3,
    ...
    excerpt:
      "Practical tips and techniques for creating sustainable digital habits that support your well-being and productivity.",
    publishedAt: "2025-01-10",
    readingTime: "6 min read",
    category: "Lifestyle",
    tags: ["habits", "productivity", "wellness"],
    image:
      "https://images.pexels.com/photos/1181244/pexels-photo-1181244.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "small",
  },
  {
    id: 4,
    ...
    excerpt:
      "How to maintain a healthy relationship with technology while staying connected and productive in today's digital world.",
    publishedAt: "2025-01-08",
    readingTime: "10 min read",
    category: "Mindfulness",
    tags: ["mindfulness", "technology", "balance"],
    image:
      "https://images.pexels.com/photos/1181298/pexels-photo-1181298.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "medium",
  },
  {
    id: 5,
    ...
    excerpt:
      "Research-backed insights into the benefits of taking breaks from digital devices and how to implement effective detox strategies.",
    publishedAt: "2025-01-05",
    readingTime: "15 min read",
    category: "Research",
    tags: ["research", "detox", "health"],
    image:
      "https://images.pexels.com/photos/1181263/pexels-photo-1181263.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "large",
  },
  {
    id: 6,
    ...
    excerpt:
      "Design strategies for establishing technology-free spaces that promote relaxation, connection, and mindful living.",
    publishedAt: "2025-01-03",
    readingTime: "7 min read",
    category: "Home Design",
    tags: ["home", "design", "wellness"],
    image:
      "https://images.pexels.com/photos/1181316/pexels-photo-1181316.jpeg?auto=compress&cs=tinysrgb&w=800&h=400&dpr=2",
    size: "small",
  },
]
...
function BlogPostSkeleton({ isListView = false, size = "medium" }: { isListView?: boolean; size?: string }) {
  if (isListView) {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex gap-4 py-6 px-4 -mx-4">
  ...
  return (
    <motion.div
      ...
      className={`${sizeClasses[size as keyof typeof sizeClasses]} h-full`}
    >
      <Card className="overflow-hidden border-neutral-800 bg-neutral-950 h-full">
        <div className={`${size === "large" ? "aspect-[2/1]" : "aspect-video"}`}>
...
function BentoGridCard({ post, isLoading = false }: { post: any; isLoading?: boolean }) {
  if (isLoading) {
    return <BlogPostSkeleton isListView={false} size={post?.size || "medium"} />
  }
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3 }}
      layout
      className={`${sizeClasses[post.size as keyof typeof sizeClasses]} h-full`}
    >
      <Card className="group overflow-hidden border-neutral-800 bg-neutral-950 hover:border-neutral-700 transition-all duration-300 hover:shadow-lg hover:shadow-black/20 h-full flex flex-col">
        <div className={`overflow-hidden ${isLarge ? "aspect-[2/1]" : "aspect-video"}`}>
          <Image
        ...
        <CardHeader className={`space-y-4 flex-1 flex flex-col ${isLarge ? "p-8" : "p-6"}`}>
          <div className="flex items-center gap-2">
          ...
          <h3
            className={`font-semibold text-white group-hover:text-neutral-200 transition-colors line-clamp-2 ${
          ...
          <p
            className={`text-neutral-400 leading-relaxed flex-1 ${
          ...
          <div className="flex flex-wrap gap-1 mt-auto">
            {post.tags.slice(0, isLarge ? 4 : 3).map((tag: string) => (
              <Badge
                key={tag}
                variant="outline"
...
function BlogPostCard({
  ...
  isLoading = false,
  isListView = false,
}: { post: any; isLoading?: boolean; isListView?: boolean }) {
  if (isLoading) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="space-y-2">
            ...
            <p className="text-neutral-400 text-sm leading-relaxed line-clamp-2">{post.excerpt}</p>
        ...
        <main className="flex-1 container mx-auto px-6 py-24">
          ...
          <div className="text-center pt-20 mb-16 font-sans">
            <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight">
              Undcitfy blog
              <span className="block text-neutral-400 text-2xl md:text-3xl font-normal mt-2">Insights & Stories</span>
            </h1>
            <p className="text-neutral-400 text-lg max-w-2xl mx-auto leading-relaxed">
              Discover articles, research, and practical guides to help you build a healthier relationship with
              technology.
            </p>
          </div>
          ...
          <Tabs value={viewMode} onValueChange={setViewMode} className="w-full font-serif">
            ...
            <TabsContent value="grid" className="mt-0">
              <motion.div
                ...
                className="mb-8"
              >
                <h2 className="text-2xl font-bold text-white mb-6 text-center">Featured Stories</h2>
              </motion.div>
              ...
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ duration: 0.3 }}
                className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-6 auto-rows-[300px]"
              >
                <AnimatePresence mode="wait">
                  {isLoading
                    ? Array.from({ length: 6 }).map((_, index) => (
                        <BentoGridCard
                          key={index}
                          post={{ size: mockPosts[index]?.size || "medium" }}
                          isLoading={true}
                        />
                      ))
                    : paginatedPosts.map((post) => <BentoGridCard key={post.id} post={post} />)}
                </AnimatePresence>
              </motion.div>
            </TabsContent>
          </Tabs>
          ...
          {!isLoading && filteredPosts.length === 0 && (
            ...
            >
              ...
              <p className="text-neutral-500 text-sm">Try adjusting your search terms or category filter.</p>
            </motion.div>
          )}
        </main>
        <Footer />
